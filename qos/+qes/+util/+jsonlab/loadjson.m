...
opt=qes.util.jsonlab.varargin2struct(varargin{:});

if(qes.util.jsonlab.jsonopt('ShowProgress',0,opt)==1)
...
function object = parse_object(inStr, esc, varargin)
    ...
    parse_char(inStr, '}');
    if(isstruct(object))
        object=qes.util.jsonlab.struct2jdata(object);
    end
...
function object = parse_array(inStr, esc, varargin) % JSON array is written in row-major order
    ...
    object = cell(0, 1);
    dim2=[];
    arraydepth=qes.util.jsonlab.jsonopt('JSONLAB_ArrayDepth_',1,varargin{:});
    pbar=-1;
    ...
    if next_char(inStr) ~= ']'
	if(qes.util.jsonlab.jsonopt('FastArrayParser',1,varargin{:})>=1 && arraydepth>=qes.util.jsonlab.jsonopt('FastArrayParser',1,varargin{:}))
        ...
        try
         ...
         while 1
            newopt=qes.util.jsonlab.varargin2struct(varargin{:},'JSONLAB_ArrayDepth_',arraydepth+1);
    ...
    if(qes.util.jsonlab.jsonopt('SimplifyCell',0,varargin{:})==1)
      try
        oldobj=object;
        object=cell2mat(object')';
        if(iscell(oldobj) && isstruct(object) && numel(object)>1 && qes.util.jsonlab.jsonopt('SimplifyCellArray',1,varargin{:})==0)
            object=oldobj;
