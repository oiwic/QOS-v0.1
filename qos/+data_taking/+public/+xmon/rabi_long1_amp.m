function varargout = rabi_long1_amp(varargin)
...
fcn_name = 'data_taking.public.xmon.rabi_long1_amp'; % this and args will be saved with data
import qes.*
import sqc.*
import sqc.op.physical.*

args = util.processArgs(varargin,{'biasAmp',0,'biasLonger',0,'dataTyp','P',...
...
x = expParam(X,'amp');
x.name = [q.name,' xy Drive amplitude'];
y = expParam(@procFactory);
y.name = [q.name,' xy Drive Pulse Length'];
s1 = sweep(x);
s1.vals = args.xyDriveAmp;
s2 = sweep(y);
