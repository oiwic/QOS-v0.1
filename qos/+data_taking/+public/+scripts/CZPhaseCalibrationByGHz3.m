function CZPhaseCalibrationByGHz3()
    import sqc.measure.*
   ...
   aczSettings2.load();
   qubits{1}.aczSettings(end+1) = aczSettings2;
   qubits{2}.aczSettings(end+1) = aczSettings2;
   
   aczSettings3 = sqc.qobj.aczSettings('q7_q6');
   aczSettings3.load();
   qubits{2}.aczSettings(end+1) = aczSettings3;
   ...
   qubits{1}.aczSettings(1).dynamicPhase(3)=phaseQ6.val;
   qubits{2}.aczSettings(1).dynamicPhase(3)=phaseQ6.val;
   qubits{2}.aczSettings(2).dynamicPhase(3)=phaseQ8.val;
   qubits{3}.aczSettings(1).dynamicPhase(3)=phaseQ8.val;
   p = sqc.op.physical.gateParser.parse(qubits,gateMat);
   p.Run();
   R = resonatorReadout(qubits);
   R.delay = p.length;
